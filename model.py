...
# Paksa TensorFlow menggunakan GPU (DirectML) sebelum operasi lain
gpus = tf.config.experimental.list_physical_devices('GPU')
if gpus:
    try:
        # Setel GPU sebagai perangkat default sebelum inisialisasi TensorFlow
        tf.config.experimental.set_visible_devices(gpus[0], 'GPU')
        tf.config.experimental.set_memory_growth(gpus[0], True)
        print("Menggunakan GPU (DirectML):", gpus[0])
    except RuntimeError as e:
        print("Gagal mengonfigurasi GPU:", e)
else:
    print("Tidak ada GPU yang tersedia. Menggunakan CPU.")

# Cek apakah TensorFlow menggunakan DirectML
...
from mrcnn.model_mobilenet import get_mobilenet_backbone
...
def compute_backbone_shapes(config, image_shape):
    ...
    assert config.BACKBONE in ["resnet50", "resnet101", "mobilenetv1", "mobilenetv2", "mobilenetv3small", "mobilenetv3large"]
    if "mobilenet" in config.BACKBONE:
        return np.array(
            [[int(math.ceil(image_shape[0] / s)), int(math.ceil(image_shape[1] / s))] 
             for s in [4, 8, 16, 32]]  # MobileNet menggunakan stride yang lebih ringan
        )
    else:
        return np.array(
...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    """Build a ResNet graph.
        ...
        train_bn: Boolean. Train or freeze Batch Norm layers
    """
    assert architecture in ["resnet50", "resnet101", "mobilenetv1", "mobilenetv2", "mobilenetv3small", "mobilenetv3large"]
    # Stage 1
...
def rpn_graph(feature_map, anchors_per_location, anchor_stride):
    ...
    rpn_bbox = KL.Lambda(lambda t: tf.reshape(t, [tf.shape(input=t)[0], -1, 4]), output_shape=(None, None, 4))(x)
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        if callable(config.BACKBONE):
            ...
            if "mobilenet" in config.BACKBONE:
                C5 = get_mobilenet_backbone(input_image, backbone_name=config.BACKBONE)
                C2, C3, C4 = C5, C5, C5  # Duplikasikan output C5 agar FPN tetap bisa berjalan
            else:
    ...
    def get_anchors(self, image_shape):
        ...
        # Pastikan jumlah feature_shapes sesuai dengan jumlah BACKBONE_STRIDES
        if "mobilenet" in self.config.BACKBONE:
            backbone_shapes = backbone_shapes[:4]  # MobileNet hanya memiliki 4 level fitur
        ...
        if not tuple(image_shape) in self._anchor_cache:
            ...
            a = utils.generate_pyramid_anchors(
                ...
                backbone_shapes,
                self.config.BACKBONE_STRIDES[:len(backbone_shapes)],  # Sesuaikan jumlah strides
